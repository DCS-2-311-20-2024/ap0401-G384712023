...
function init() {
  ...
  const vFrameW = 0.5;  const vFrameH = 1.2;  const vFrameD = 22;
  {
    ...
    lFrame.position.x = -(vFrameW + hFrameW) / 2.15;
    scene.add(lFrame);

    //   右の枠
    const rFrame = lFrame.clone();
    rFrame.position.x = (vFrameW + hFrameW) / 2.15;
    scene.add(rFrame);
  }
  ...
  const paddle = new THREE.Group(); // パドルグループ
  {
    ...
    right.position.x = paddleR * 2.5;
    paddle.add(right);
    // パドル端(左)
    const left = right.clone();
    left.rotation.z = 110;
    left.position.x = paddleR * -2.45;
    paddle.add(left);
    // パドルの配置
  ...
  function remakeBricks(){
    ...
    bricks.clear(); //ブロックからのグループを空にする
    nBrick = 0; 
    makeBricks(); //新しい数のブロックを作る
    scene.add(bricks); //シーンに追加
  }
